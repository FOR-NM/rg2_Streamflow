...
rating_data <- BRAA1 %>% 
  filter(!is.na(Baro_Cor_offset3), !is.na(Q_L_per_s))
...
########################################
ggplot(data = BRAA1_baro, aes(x = DateTime, y = Baro_Cor_offset3)) +
...
rating_data <- rating_data %>% 
  filter(!Date.y %in% c("2025-05-12", "2025-05-15", "2024-09-13", "2024-10-11"))
...
plot(BRAA1$Baro_Cor_offset3, BRAA1$Predicted_Discharge_Log.m3s, col = "red", type = "l", lwd = 2,
     xlab = "Stage (m)", ylab = "Discharge (m³/s)", main = "Discharge Predictions")
lines(BRAA1$Baro_Cor_offset3, BRAA1$Predicted_Discharge_Linear.m3s, col = "green", lwd = 2)
...
ggplot(BRAA1, aes(x = Baro_Cor_offset3)) +
