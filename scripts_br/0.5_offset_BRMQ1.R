...
ggplot(data = BRMQ1_baro, aes(x = DateTime, y = prcp)) +
  geom_line() + ggtitle("BRMQ1 level data in m")

# Plot baro compensated with precipitation
plot_ly(BRMQ1_baro, x = ~DateTime) %>%
...
###################################################
#### Plot before and after Telemetry data data ####
###################################################
# add a column for before/after March 14
rating_data <- rating_data %>%
  mutate(
    Date.x = as.Date(Date.x),  # ensure it's a proper Date
    Period = if_else(Date.x < as.Date("2025-03-14"), "Before March 14", "After March 14")
  )
# Plot divided by before/after March 14
p <- ggplot(rating_data, aes(x = Baro_Cor_Lvl.m, y = Q.m3s, color = Period)) +
  geom_point(size = 3) +
  geom_text(aes(label = Date.x), vjust = -0.5, size = 3, show.legend = FALSE) +
  labs(
    title = "Stage vs. Discharge (Before and After March 14)",
    x = "Stage (LEVEL m)",
    y = "Discharge (Q m³/s)",
    color = "Period"
  ) +
  scale_color_manual(values = c("Before March 14" = "#1f77b4", "After March 14" = "#ff7f0e")) +
  theme_minimal()
...
#######################################################
ggplot(BRMQ1, aes(x = DateTime, y = Baro_Cor_Lvl.m)) +
  geom_line() +
  geom_vline(xintercept = as.POSIXct("2025-03-14 08:00:00"), linetype="dashed", color="red") +
  geom_vline(xintercept = as.POSIXct("2025-06-26 08:00:00"), linetype="dashed", color="red") +
  labs(title = "Baro_Cor_Lvl", x = "Date", y = "Water Level (m)")

# the vented PT was put out March 14th but then we lost it in a storm in mid June! So the PT deployed June 26 is not vented
...
Date1 <- as.Date("2024-10-15", "%Y-%m-%d")
...
subdf <- BRMQ1[BRMQ1$DateTime < Date2 & BRMQ1$DateTime > Date1,]
ggplot(data=subdf, aes(DateTime,Baro_Cor_Lvl.m)) + geom_line() +
  geom_vline(xintercept = as.POSIXct("2025-01-24 09:00:00"), linetype="dashed", color="red") 

Date1 <- as.Date("2025-03-01", "%Y-%m-%d")
Date2 <- as.Date("2025-03-10", "%Y-%m-%d")
subdf <- BRMQ1[BRMQ1$DateTime < Date2 & BRMQ1$DateTime > Date1,]
ggplot(data=subdf, aes(DateTime,Baro_Cor_Lvl.m)) + geom_line() +
  geom_vline(xintercept = as.POSIXct("2025-03-04 13:00:00"), linetype="dashed", color="red") 

Date1 <- as.Date("2025-04-10", "%Y-%m-%d")
Date2 <- as.Date("2025-04-20", "%Y-%m-%d")
subdf <- BRMQ1[BRMQ1$DateTime < Date2 & BRMQ1$DateTime > Date1,]
ggplot(data=subdf, aes(DateTime,Baro_Cor_Lvl.m)) + geom_line() +
  geom_vline(xintercept = as.POSIXct("2025-04-11 12:00:00"), linetype="dashed", color="red") +
  geom_vline(xintercept = as.POSIXct("2025-04-11 17:30:00"), linetype="dashed", color="red")
...
subdf <- BRMQ1[BRMQ1$DateTime < Date2 & BRMQ1$DateTime > Date1,]
ggplot(data=subdf, aes(DateTime,Baro_Cor_Lvl.m)) + geom_line() +
  geom_vline(xintercept = as.POSIXct("2025-07-12 09:00:00"), linetype="dashed", color="red") 
...
Date1 <- as.Date("2025-03-13", "%Y-%m-%d")
...
move_time1 <- as.POSIXct("2024-10-11 11:00:00")  
after_move <- BRMQ1 %>%
...
move_time5 <- as.POSIXct("2025-04-11 11:30:00")
before_move <- BRMQ1 %>%
...
move_time6 <- as.POSIXct("2025-04-11 18:00:00")
before_move <- BRMQ1 %>%
...
move_time7 <- as.POSIXct("2025-04-12 00:00:00")
before_move <- BRMQ1 %>%
...
ggplot(data=subdf, aes(DateTime,Baro_Cor_Lvl.m)) + geom_line() +
  geom_vline(xintercept = as.POSIXct("2025-04-11 12:00:00"), linetype="dashed", color="red") +
  geom_vline(xintercept = as.POSIXct("2025-04-11 17:30:00"), linetype="dashed", color="red") +
  geom_vline(xintercept = as.POSIXct("2025-04-12 00:00:00"), linetype="dashed", color="red")
ggplot(data=subdf, aes(DateTime,Baro_Cor_offset7)) + geom_line() +
  geom_vline(xintercept = as.POSIXct("2025-04-11 17:30:00"), linetype="dashed", color="red") +
  geom_vline(xintercept = as.POSIXct("2025-04-11 12:00:00"), linetype="dashed", color="red")

move_time8 <- as.POSIXct("2025-06-26 12:15:00")
before_move <- BRMQ1 %>%
...
# compute offset
offset8 <- after_move$mean_after - before_move$mean_before + 0.3
print(offset8)
# apply the sixth correction
BRMQ1 <- BRMQ1 %>%
...
ggplot(BRMQ1, aes(x = DateTime, y = Baro_Cor_offset2)) +
  geom_line() +
  #geom_vline(xintercept = as.POSIXct("2024-07-30 15:10:00"), linetype="dashed", color="red") +
  labs(title = "Corrected Baro_Cor Over Time", x = "Date", y = "Water Level (m)")
...
ggplot(BRMQ1, aes(x = DateTime, y = Baro_Cor_offset4)) +
  ...
  #geom_vline(xintercept = as.POSIXct("2024-07-30 15:10:00"), linetype="dashed", color="red") +
  labs(title = "Corrected Baro_Cor Over Time", x = "Date", y = "Water Level (m)")
ggplot(BRMQ1, aes(x = DateTime, y = Baro_Cor_offset5)) +
  geom_line() +
  #geom_vline(xintercept = as.POSIXct("2024-07-30 15:10:00"), linetype="dashed", color="red") +
  labs(title = "Corrected Baro_Cor Over Time", x = "Date", y = "Water Level (m)")
ggplot(BRMQ1, aes(x = DateTime, y = Baro_Cor_offset6)) +
  geom_line() +
  #geom_vline(xintercept = as.POSIXct("2024-07-30 15:10:00"), linetype="dashed", color="red") +
  labs(title = "Corrected Baro_Cor Over Time", x = "Date", y = "Water Level (m)")
ggplot(BRMQ1, aes(x = DateTime, y = Baro_Cor_offset7)) +
  geom_line() +
  #geom_vline(xintercept = as.POSIXct("2024-07-30 15:10:00"), linetype="dashed", color="red") +
  labs(title = "Corrected Baro_Cor Over Time", x = "Date", y = "Water Level (m)")
ggplot(BRMQ1, aes(x = DateTime, y = Baro_Cor_offset8)) +
  geom_line() +
  #geom_vline(xintercept = as.POSIXct("2024-07-30 15:10:00"), linetype="dashed", color="red") +
  labs(title = "Corrected Baro_Cor Over Time", x = "Date", y = "Water Level (m)")
...
BRMQ1$Q_L_per_s <- as.numeric(BRMQ1$Q_L_per_s)
BRMQ1 <- BRMQ1 %>%
  mutate(Q.m3s = Q_L_per_s/1000)
# filter out rows with missing stage or discharge
rating_data_offset <- BRMQ1 %>% 
...
p <- ggplot(rating_data_offset, aes(x = Baro_Cor_offset3, y = Q.m3s, color = Season)) +
  ...
  geom_text(aes(label = Date.x), vjust = -0.5, size = 3, show.legend = FALSE) +
  labs(
    title = "Stage vs. Discharge by Season - BRMQ1",
    x = "Stage (LEVEL m)",
    y = "Discharge (Q m³/s)",
    color = "Season"
  ) +
...
#Automatically save plot
ggsave(filename = "br_figs/Season_BRMQ1.png", plot = p,
       width = 8, height  = 6, dpi = 300)
...
#Automatically save plot
ggsave(filename = "br_figs/SaltSlug_vs_FlowMeter_BRMQ1.png", plot = p,
       width = 8, height  = 6, dpi = 300)
...
Date1 <- as.Date("2025-03-10", "%Y-%m-%d")
Date2 <- as.Date("2025-06-30", "%Y-%m-%d")
subdf <- BRMQ1[BRMQ1$DateTime < Date2 & BRMQ1$DateTime > Date1,]
ggplot(data=subdf, aes(DateTime,Baro_Cor_Lvl.m)) + geom_line() +
  geom_vline(xintercept = as.POSIXct("2025-06-26 21:00:00"), linetype="dashed", color="red") 
ggplot(data=subdf, aes(DateTime,Baro_Cor_offset7)) + geom_line() +
  geom_vline(xintercept = as.POSIXct("2025-06-26 21:00:00"), linetype="dashed", color="red") 
...
#### Save file ####
###################
write.csv(BRMQ1, "data/offset_BRMQ1.csv")

drive_folder_id <- "1E3pAdlfgxluBGmYT4r95obkaAduKIXhQ"

# upload file to the specified Google Drive folder
drive_put(
  media = "data/offset_BRMQ1.csv",
