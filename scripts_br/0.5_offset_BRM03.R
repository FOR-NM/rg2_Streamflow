##==============================================================================
## Project: QuEST
## This script is to calculate PT offsed for Brush Creek BRM03 site
...
#BRM03
googledrive::drive_download(file = pt_csvs$id[pt_csvs$name=="BRM03.csv"], 
                            path = "googledrive/BRM03.csv",
                            overwrite = T)
# load file
BRM03 <- read.csv("googledrive/BRM03.csv")

# combine Date and Time columns into a new DateTime column
BRM03$DateTime <- paste(BRM03$Date.x, BRM03$Time.x, sep = " ")

# convert the DateTime column to POSIXct
BRM03$DateTime <- as.POSIXct(BRM03$DateTime, format = "%Y-%m-%d %H:%M:%S")
...
ggplot(data = BRM03_baro, aes(x = DateTime, y = Baro_Cor_Lvl.m)) +
...
subdf <- BRM03[BRM03$DateTime < Date2 & BRM03$DateTime > Date1,]
...
subdf <- BRM03[BRM03$DateTime < Date2 & BRM03$DateTime > Date1,]
ggplot(data=subdf, aes(DateTime,Baro_Cor_Lvl.m)) + geom_line() +
  geom_vline(xintercept = as.POSIXct("2025-05-12 11:00:00"), linetype="dashed", color="red") 
...
time1 <- as.POSIXct("2025-03-10 08:00:00")
...
###############################
ggplot(BRM03, aes(x = DateTime, y = Baro_Cor_Lvl.m)) +
  geom_line() +
...
ggplot(BRM03, aes(x = DateTime, y = Baro_Cor_offset1)) +
  geom_line() +
  #geom_vline(xintercept = as.POSIXct("2024-07-30 15:10:00"), linetype="dashed", color="red") +
  labs(title = "Corrected Baro_Cor Over Time", x = "Date", y = "Water Level (m)")
# ggplot(BRM03, aes(x = DateTime, y = Baro_Cor_offset2)) +
#   geom_line() +
#   #geom_vline(xintercept = as.POSIXct("2024-07-30 15:10:00"), linetype="dashed", color="red") +
#   labs(title = "Corrected Baro_Cor Over Time", x = "Date", y = "Water Level (m)")
# ggplot(BRM03, aes(x = DateTime, y = Baro_Cor_offset3)) +
#   geom_line() +
#   #geom_vline(xintercept = as.POSIXct("2024-07-30 15:10:00"), linetype="dashed", color="red") +
#   labs(title = "Corrected Baro_Cor Over Time", x = "Date", y = "Water Level (m)")
# # ggplot(BRM03, aes(x = DateTime, y = Baro_Cor_offset5)) +
#   geom_line() +
#   #geom_vline(xintercept = as.POSIXct("2024-07-30 15:10:00"), linetype="dashed", color="red") +
#   labs(title = "Corrected Baro_Cor Over Time", x = "Date", y = "Water Level (m)")
# ggplot(BRM03, aes(x = DateTime, y = Baro_Cor_offset6)) +
#   geom_line() +
#   #geom_vline(xintercept = as.POSIXct("2024-07-30 15:10:00"), linetype="dashed", color="red") +
#   labs(title = "Corrected Baro_Cor Over Time", x = "Date", y = "Water Level (m)")
# ggplot(BRM03, aes(x = DateTime, y = Baro_Cor_offset7)) +
#   geom_line() +
#   #geom_vline(xintercept = as.POSIXct("2024-07-30 15:10:00"), linetype="dashed", color="red") +
#   labs(title = "Corrected Baro_Cor Over Time", x = "Date", y = "Water Level (m)")
...
BRM03$Q_L_per_s <- as.numeric(BRM03$Q_L_per_s)
BRM03 <- BRM03 %>%
  mutate(Q.m3s = Q_L_per_s/1000)
# filter out rows with missing stage or discharge
rating_data_offset <- BRM03 %>% 
...
p <- ggplot(rating_data_offset, aes(x = Baro_Cor_offset1, y = Q.m3s, color = Season)) +
  ...
  labs(
    title = "Stage vs. Discharge by Season - BRM03",
    x = "Stage (LEVEL m)",
    y = "Discharge (Q m³/s)",
    color = "Season"
  ) +
...
ggsave(filename = "br_figs/Season_BRM03.png", plot = p,
...
p <- ggplot(rating_data_offset, aes(x = Baro_Cor_offset1, y = Q.m3s, color = Period)) +
  geom_point(size = 3) +
  geom_text(aes(label = Date.x), vjust = -0.5, size = 3, show.legend = FALSE) +
  labs(
...
#Automatically save plot
ggsave(filename = "br_figs/April13_BRM03.png", plot = p,
       width = 8, height  = 6, dpi = 300)
...
p <- ggplot(rating_data_offset, aes(x = Baro_Cor_offset1, y = Q.m3s, color = Q_type)) +
  geom_point(size = 3) +
  geom_text(aes(label = Date.x), vjust = -0.5, size = 3, show.legend = FALSE) +
  labs(
...
#Automatically save plot
ggsave(filename = "br_figs/SaltSlug_vs_FlowMeter_BRM03.png", plot = p,
       width = 8, height  = 6, dpi = 300)
...
#### Save file ####
###################
write.csv(BRM03, "data/offset_BRM03.csv")

drive_folder_id <- "1E3pAdlfgxluBGmYT4r95obkaAduKIXhQ"

# upload file to the specified Google Drive folder
drive_put(
  media = "data/offset_BRM03.csv",
