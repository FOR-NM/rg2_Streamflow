...
rating_data <- BRA01 %>% 
  filter(!is.na(Baro_Cor_offset2), !is.na(Q_L_per_s))
...
BRA01_baro <- BRA01 %>% 
  filter(!is.na(Baro_Cor_offset2))

########################################
#### Plot pressure compensated data ####
########################################
ggplot(data = BRA01_baro, aes(x = DateTime, y = Baro_Cor_offset2)) +
...
BRA01 <- BRA01 %>%
  mutate(Predicted_Discharge_Log.m3s = a_log * (Baro_Cor_offset2 ^ b_log))
...
# predict discharge for the entire dataset
BRA01 <- BRA01 %>%
  mutate(Predicted_Discharge_Linear.m3s = a_linear + b_linear * Baro_Cor_offset2)

#############################
#### Compare predictions ####
#############################
# visualize predictions
plot(BRA01$Baro_Cor_offset2, BRA01$Predicted_Discharge_Log.m3s, col = "red", type = "l", lwd = 2,
     xlab = "Stage (m)", ylab = "Discharge (m³/s)", main = "Discharge Predictions")
lines(BRA01$Baro_Cor_offset2, BRA01$Predicted_Discharge_Linear.m3s, col = "green", lwd = 2)
...
ggplot(BRA01, aes(x = Baro_Cor_offset2)) +
