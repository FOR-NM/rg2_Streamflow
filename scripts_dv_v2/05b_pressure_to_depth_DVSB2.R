...
## produce a true Final_Water_Depth_m timeseries for DVSB2.
##
## INPUT:  "offset" Google Drive folder (output of script 05) -> offset_DVSB2.csv
...
googledrive::drive_download(
  file = pt_csvs$id[pt_csvs$name == "offset_DVSB2.csv"],
...
DVSB2$Final_Corrected_Lvl  <- suppressWarnings(as.numeric(DVSB2$Final_Corrected_Lvl))
DVSB2$Actual_Water_Depth_m <- suppressWarnings(as.numeric(DVSB2$Actual_Water_Depth_m))

cat("Final_Corrected_Lvl NAs:  ", sum(is.na(DVSB2$Final_Corrected_Lvl)), "\n")
cat("Actual_Water_Depth_m NAs: ", sum(is.na(DVSB2$Actual_Water_Depth_m)), "\n")
...
cat("Number of paired depth observations for DVSB2:", nrow(level_data), "\n")
...
if (nrow(level_data) >= min_obs_for_model) {
  ...
  cat("\n--- Depth conversion model summary (DVSB2) ---\n")
  ...
  if (is.na(sensor_height_m)) {
    stop("Not enough paired observations to fit a depth model for DVSB2,
...
ggplot(level_data, aes(x = Actual_Water_Depth_m, y = Predicted_Depth_m)) +
  ...
  labs(
    title = "DVSB2: Observed vs. Predicted Water Depth",
...
ggplot(level_data, aes(x = Final_Corrected_Lvl, y = Actual_Water_Depth_m)) +
  ...
  geom_text(aes(label = format(as.Date(Date.x), "%Y-%m-%d")), vjust = -0.7, size = 3) +
  labs(
    title = "DVSB2: Offset-corrected pressure vs. actual field depth",
...
ggplot(DVSB2, aes(x = DateTime, y = Final_Water_Depth_m)) +
  geom_line(color = "steelblue") +
  labs(
    title = "DVSB2: Water Depth timeseries (converted from pressure)",
    x = "Date",
    y = "Water Depth (m)"
  ) +
  theme_minimal()
...
ggplot(DVSB2, aes(x = DateTime)) +
  ...
  labs(
    title = "DVSB2: Pressure vs. depth comparison",
...
n_negative <- sum(DVSB2$Final_Water_Depth_m < 0, na.rm = TRUE)
if (n_negative > 0) {
  cat(sprintf(
...
#### Save file ####
###################
write.csv(DVSB2, "data/depth_corrected_DVSB2.csv", row.names = FALSE)

# this is the "depth_corrected" folder — create this folder in Drive first
# and paste its ID here (reusing the folder originally set up for DVSB2)
drive_folder_id <- "1wlpAOSpJyB6LI2XYkRXRZwhXRkXqe5eG"  # <-- confirm/replace with your Drive folder ID

drive_put(
  media = "data/depth_corrected_DVSB2.csv",
