...
#### Plot air data ####
#######################
ggplot(data = air_upper, aes(x = DateTime, y = Level_air.kPa)) + 
  geom_line() + ggtitle("Air upper") 

ggplot(data = air_lower, aes(x = DateTime, y = Level_air.kPa)) + 
...
for (i in names(merged_upper)) {
  ...
  # compensate
  df <- df %>%
    mutate(Baro_Cor_Lvl.kPa = (df$LEVEL.kPa - df$Level_air.kPa))
...
for (i in seq_along(compensated_lower)) {
  ...
  # Plot
  p <- ggplot(data = df, aes(x = DateTime, y = Baro_Cor_Lvl.kPa)) + 
...
for (i in seq_along(compensated_upper)) {
  df <- compensated_upper[[i]]
  df$Baro_Cor_Lvl.m <- df$Baro_Cor_Lvl.kPa * 0.101972
  compensated_upper[[i]] <- df
}
for (i in seq_along(compensated_lower)) {
  df <- compensated_lower[[i]]
