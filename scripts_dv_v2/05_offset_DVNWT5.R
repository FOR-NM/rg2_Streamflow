...
DVNWT5$Baro_backup <- DVNWT5$Baro_Cor_Lvl.m
...
########################################
ggplot(data = DVNWT5, aes(x = DateTime, y = Baro_Cor_Lvl.m)) +
...
ggplot(DVNWT5, aes(x = DateTime, y = Baro_Cor_Lvl.m)) +
  geom_line() +
  geom_vline(xintercept = as.POSIXct("2024-11-19"), linetype="dashed", color="blue") + # "sensor moved"
  geom_vline(xintercept = as.POSIXct("2025-04-18"), linetype="dashed", color="red") + # sensor change
  geom_vline(xintercept = as.POSIXct("2024-10-18"), linetype="dashed", color="red") + # sensor broke
...
DVNWT5 <- DVNWT5 %>%
  mutate(Baro_Cor_Lvl.m = ifelse(DateTime == time8, NA, Baro_Cor_Lvl.m))
DVNWT5 <- DVNWT5 %>%
  mutate(Baro_Cor_Lvl.m = ifelse(DateTime == time9, NA, Baro_Cor_Lvl.m))

# remove first couple data points in start of data
Date1 <- as.Date("2024-07-30", "%Y-%m-%d")
Date2 <- as.Date("2024-08-02", "%Y-%m-%d")
DVNWT5$Baro_Cor_Lvl.m[DVNWT5$DateTime >= Date1 & DVNWT5$DateTime <= Date2] <- NA
...
move_time1 <- as.POSIXct("2025-04-25 12:00:00")
before_move1 <- DVNWT5 %>%
...
ggplot(new_level_data, aes(x = Baro_Cor_Lvl.m, y = Actual_Water_Depth_m)) +
  geom_point(color = "blue") +
...
ggplot(new_level_data, aes(x = Baro_Cor_offset1, y = Actual_Water_Depth_m)) +
  geom_point(color = "blue") +
  geom_text(aes(label = Date.x), vjust = -0.5, size = 3) +
...
ggplot(new_level_data, aes(x = Baro_Cor_offset1, y = Actual_Water_Depth_m)) +
  geom_point(color = "blue") +
  geom_text(aes(label = Date.x), vjust = -0.5, size = 3) +
  labs(title = "Baro-corrected level vs. Actual water depth at sensor (Final Correction)", x = "Baro-corrected level (m)", y = "Water depth  (m)") +
  theme_minimal()
...
# site-specific corrections were applied. NOTE: Baro_Cor_offset5 is the last
# correction actually computed above (offset6 is still commented out).
DVNWT5 <- DVNWT5 %>%
  mutate(Final_Corrected_Lvl = Baro_Cor_offset1)
