...
## produce a true Final_Water_Depth_m timeseries for DVMS5.
##
## INPUT:  "offset" Google Drive folder (output of script 05) -> offset_DVMS5.csv
...
googledrive::drive_download(
  file = pt_csvs$id[pt_csvs$name == "offset_DVMS5.csv"],
  path = "googledrive/offset_DVMS5.csv",
  overwrite = TRUE
)

DVMS5 <- read.csv("googledrive/offset_DVMS5.csv")
...
DVMS5$Final_Corrected_Lvl  <- suppressWarnings(as.numeric(DVMS5$Final_Corrected_Lvl))
DVMS5$Actual_Water_Depth_m <- suppressWarnings(as.numeric(DVMS5$Actual_Water_Depth_m))

cat("Final_Corrected_Lvl NAs:  ", sum(is.na(DVMS5$Final_Corrected_Lvl)), "\n")
cat("Actual_Water_Depth_m NAs: ", sum(is.na(DVMS5$Actual_Water_Depth_m)), "\n")
...
cat("Number of paired depth observations for DVMS5:", nrow(level_data), "\n")
...
if (nrow(level_data) >= min_obs_for_model) {
  ...
  cat("\n--- Depth conversion model summary (DVMS5) ---\n")
  ...
  if (is.na(sensor_height_m)) {
    stop("Not enough paired observations to fit a depth model for DVMS5,
...
ggplot(level_data, aes(x = Actual_Water_Depth_m, y = Predicted_Depth_m)) +
  ...
  labs(
    title = "DVMS5: Observed vs. Predicted Water Depth",
...
ggplot(level_data, aes(x = Final_Corrected_Lvl, y = Actual_Water_Depth_m)) +
  ...
  geom_text(aes(label = format(as.Date(Date.x), "%Y-%m-%d")), vjust = -0.7, size = 3) +
  labs(
    title = "DVMS5: Offset-corrected pressure vs. actual field depth",
...
ggplot(DVMS5, aes(x = DateTime, y = Final_Water_Depth_m)) +
  geom_line(color = "steelblue") +
  labs(
    title = "DVMS5: Water Depth timeseries (converted from pressure)",
    x = "Date",
    y = "Water Depth (m)"
  ) +
  theme_minimal()
...
ggplot(DVMS5, aes(x = DateTime)) +
  ...
  labs(
    title = "DVMS5: Pressure vs. depth comparison",
...
n_negative <- sum(DVMS5$Final_Water_Depth_m < 0, na.rm = TRUE)
if (n_negative > 0) {
  cat(sprintf(
...
#### Save file ####
###################
write.csv(DVMS5, "data/depth_corrected_DVMS5.csv", row.names = FALSE)

# this is the "depth_corrected" folder — create this folder in Drive first
# and paste its ID here (reusing the folder originally set up for DVMS5)
drive_folder_id <- "1wlpAOSpJyB6LI2XYkRXRZwhXRkXqe5eG"  # <-- confirm/replace with your Drive folder ID

drive_put(
  media = "data/depth_corrected_DVMS5.csv",
