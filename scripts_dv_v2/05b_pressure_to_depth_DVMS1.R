...
## produce a true Final_Water_Depth_m timeseries for DVMS1.
##
## INPUT:  "offset" Google Drive folder (output of script 05) -> offset_DVMS1.csv
...
googledrive::drive_download(
  file = pt_csvs$id[pt_csvs$name == "offset_DVMS1.csv"],
  path = "googledrive/offset_DVMS1.csv",
  overwrite = TRUE
)

DVMS1 <- read.csv("googledrive/offset_DVMS1.csv")
...
DVMS1$Final_Corrected_Lvl  <- suppressWarnings(as.numeric(DVMS1$Final_Corrected_Lvl))
DVMS1$Actual_Water_Depth_m <- suppressWarnings(as.numeric(DVMS1$Actual_Water_Depth_m))
...
cat("Number of paired depth observations for DVMS1:", nrow(level_data), "\n")
...
if (nrow(level_data) >= min_obs_for_model) {
  ...
  cat("\n--- Depth conversion model summary (DVMS1) ---\n")
  ...
  if (is.na(sensor_height_m)) {
    stop("Not enough paired observations to fit a depth model for DVMS1,
...
ggplot(level_data, aes(x = Actual_Water_Depth_m, y = Predicted_Depth_m)) +
  ...
  labs(
    title = "DVMS1: Observed vs. Predicted Water Depth",
...
ggplot(level_data, aes(x = Final_Corrected_Lvl, y = Actual_Water_Depth_m)) +
  ...
  geom_text(aes(label = format(as.Date(Date.x), "%Y-%m-%d")), vjust = -0.7, size = 3) +
  labs(
    title = "DVMS1: Offset-corrected pressure vs. actual field depth",
...
ggplot(DVMS1, aes(x = DateTime, y = Final_Water_Depth_m)) +
  geom_line(color = "steelblue") +
  labs(
    title = "DVMS1: Water Depth timeseries (converted from pressure)",
    x = "Date",
    y = "Water Depth (m)"
  ) +
  theme_minimal()
...
ggplot(DVMS1, aes(x = DateTime)) +
  ...
  labs(
    title = "DVMS1: Pressure vs. depth comparison",
...
n_negative <- sum(DVMS1$Final_Water_Depth_m < 0, na.rm = TRUE)
if (n_negative > 0) {
  cat(sprintf(
...
#### Save file ####
###################
write.csv(DVMS1, "data/depth_corrected_DVMS1.csv", row.names = FALSE)

# this is the "depth_corrected" folder — create this folder in Drive first
# and paste its ID here (reusing the folder originally set up for DVMS1)
drive_folder_id <- "1wlpAOSpJyB6LI2XYkRXRZwhXRkXqe5eG"  # <-- confirm/replace with your Drive folder ID

drive_put(
  media = "data/depth_corrected_DVMS1.csv",
