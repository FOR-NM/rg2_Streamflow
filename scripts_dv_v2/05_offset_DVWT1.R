##==============================================================================
## Project: QuEST
## This script is to calculate PT offset for Dog Valley DVWT1 site
...
#DVWT1
googledrive::drive_download(file = pt_csvs$id[pt_csvs$name=="DVWT1.csv"], 
                            path = "googledrive/DVWT1.csv",
                            overwrite = T)
# load file
DVWT1 <- read.csv("googledrive/DVWT1.csv")

# convert Date column to Date type if not already
# combine Date and Time columns into a new DateTime column
DVWT1$DateTime <- paste(DVWT1$Date.x, DVWT1$TimeOnly, sep = " ")
...
DVWT1$Baro_backup <- DVWT1$Baro_Cor_Lvl.m

########################################
#### Plot pressure compensated data ####
########################################
ggplot(data = DVWT1, aes(x = DateTime, y = Baro_Cor_Lvl.m)) +
  geom_line() + ggtitle("DVWT1 compensated level data")

ggplot(data = DVWT1, aes(x = DateTime, y = LEVEL.m)) +
  geom_line() + ggtitle("DVWT1 level data")
...
################################################# 
ggplot(DVWT1, aes(x = DateTime, y = LEVEL.m)) +
  geom_line() +
...
ggplot(DVWT1, aes(x = DateTime, y = Baro_Cor_Lvl.m)) +
  geom_line() +
  geom_vline(xintercept = as.POSIXct("2024-11-19"), linetype="dashed", color="red") + # cut ziptie loop
  geom_vline(xintercept = as.POSIXct("2025-02-28"), linetype="dashed", color="red") + # Levelogger burried upon arrival. Adjusted up by 9.0 cm. Levelogger redeployed at 11:30:00 AM.
  geom_vline(xintercept = as.POSIXct("2025-06-03"), linetype="dashed", color="red") + # Level logger moved down 1.0 cm using the fence post pounder
...
Date2 <- as.Date("2025-07-02", "%Y-%m-%d")
DVWT1$Baro_Cor_Lvl.m[DVWT1$DateTime >= Date1 & DVWT1$DateTime <= Date2] <- NA
...
DVWT1$Q..L.s. <- as.numeric(DVWT1$Q..L.s.)
DVWT1 <- DVWT1 %>%
  mutate(Q.m3s = Q..L.s./1000)

# filter out rows with missing stage or discharge
new_rating_data <- DVWT1 %>% 
...
#### Save file ####
###################
write.csv(DVWT1, "data/offset_DVWT1.csv")
