##==============================================================================
## Project: QuEST
## This script is to calculate discharge from compensated pressure data for Santa Fe DVNWT4 site
##==============================================================================
...
#DVNWT4
googledrive::drive_download(file = pt_csvs$id[pt_csvs$name=="depth_corrected_DVNWT4.csv"],
                            path = "googledrive/depth_corrected_DVNWT4.csv",
                            overwrite = T)
# load file
DVNWT4 <- read.csv("googledrive/depth_corrected_DVNWT4.csv")

# convert Date column to Date type if not already
DVNWT4$Date <- as.Date(DVNWT4$Date.x)
...
########################################
ggplot(data = DVNWT4, aes(x = DateTime, y = Final_Water_Depth_m)) +
...
plot(DVNWT4$Final_Water_Depth_m, DVNWT4$Predicted_Discharge_Log_m3s, col = "red", type = "l", lwd = 2,
     xlab = "Stage (m)", ylab = "Discharge (m³/s)", main = "Discharge Predictions")
lines(DVNWT4$Final_Water_Depth_m, DVNWT4$Predicted_Discharge_Linear_m3s, col = "green", lwd = 2)
...
ggplot(DVNWT4, aes(x = Final_Water_Depth_m)) +
...
######################################
DVNWT4$DateTime <- as.POSIXct(DVNWT4$DateTime)
...
#### Save file ####
###################
write.csv(DVNWT4, "data/discharge_DVNWT4.csv")
...
drive_put(
  media = "data/discharge_DVNWT4.csv",
