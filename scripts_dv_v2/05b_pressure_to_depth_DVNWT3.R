...
## produce a true Final_Water_Depth_m timeseries for DVNWT3.
##
## INPUT:  "offset" Google Drive folder (output of script 05) -> offset_DVNWT3.csv
...
# download DVNWT3 only
googledrive::drive_download(
  file = pt_csvs$id[pt_csvs$name == "offset_DVNWT3.csv"],
  path = "googledrive/offset_DVNWT3.csv",
  overwrite = TRUE
)

DVNWT3 <- read.csv("googledrive/offset_DVNWT3.csv")
...
DVNWT3$Final_Corrected_Lvl  <- suppressWarnings(as.numeric(DVNWT3$Final_Corrected_Lvl))
DVNWT3$Actual_Water_Depth_m <- suppressWarnings(as.numeric(DVNWT3$Actual_Water_Depth_m))

cat("Final_Corrected_Lvl NAs:  ", sum(is.na(DVNWT3$Final_Corrected_Lvl)), "\n")
cat("Actual_Water_Depth_m NAs: ", sum(is.na(DVNWT3$Actual_Water_Depth_m)), "\n")
...
cat("Number of paired depth observations for DVNWT3:", nrow(level_data), "\n")
...
if (nrow(level_data) >= min_obs_for_model) {
  ...
  cat("\n--- Depth conversion model summary (DVNWT3) ---\n")
  ...
  if (is.na(sensor_height_m)) {
    stop("Not enough paired observations to fit a depth model for DVNWT3,
...
ggplot(level_data, aes(x = Actual_Water_Depth_m, y = Predicted_Depth_m)) +
  ...
  labs(
    title = "DVNWT3: Observed vs. Predicted Water Depth",
...
ggplot(level_data, aes(x = Final_Corrected_Lvl, y = Actual_Water_Depth_m)) +
  ...
  geom_text(aes(label = format(as.Date(Date.x), "%Y-%m-%d")), vjust = -0.7, size = 3) +
  labs(
    title = "DVNWT3: Offset-corrected pressure vs. actual field depth",
...
ggplot(DVNWT3, aes(x = DateTime, y = Final_Water_Depth_m)) +
  geom_line(color = "steelblue") +
  labs(
    title = "DVNWT3: Water Depth timeseries (converted from pressure)",
    x = "Date",
    y = "Water Depth (m)"
  ) +
  theme_minimal()
...
ggplot(DVNWT3, aes(x = DateTime)) +
  ...
  labs(
    title = "DVNWT3: Pressure vs. depth comparison",
...
n_negative <- sum(DVNWT3$Final_Water_Depth_m < 0, na.rm = TRUE)
if (n_negative > 0) {
  cat(sprintf(
...
#### Save file ####
###################
write.csv(DVNWT3, "data/depth_corrected_DVNWT3.csv", row.names = FALSE)

# this is the "depth_corrected" folder — create this folder in Drive first
# and paste its ID here (reusing the folder originally set up for DVNWT3)
drive_folder_id <- "1wlpAOSpJyB6LI2XYkRXRZwhXRkXqe5eG"  # <-- confirm/replace with your Drive folder ID

drive_put(
  media = "data/depth_corrected_DVNWT3.csv",
