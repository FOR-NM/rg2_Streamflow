##==============================================================================
## Project: QuEST
## This script is to calculate discharge from compensated pressure data for Santa Fe DVNWT5 site
##==============================================================================
...
# DVNWT5
googledrive::drive_download(file = pt_csvs$id[pt_csvs$name=="depth_corrected_DVNWT5.csv"],
                            path = "googledrive/depth_corrected_DVNWT5.csv",
                            overwrite = T)
# load file
DVNWT5 <- read.csv("googledrive/depth_corrected_DVNWT5.csv")

# convert Date column to Date type if not already
DVNWT5$Date <- as.Date(DVNWT5$Date.x)
...
rating_data <- DVNWT5 %>% 
  filter(!is.na(Final_Water_Depth_m), !is.na(Q), !Q == 0)

write.csv(rating_data, "DVNWT5_rating.csv")
...
########################################
ggplot(data = DVNWT5, aes(x = DateTime, y = Final_Water_Depth_m)) +
...
ggplot(rating_data, aes(x = Final_Water_Depth_m, y = Q.m3s)) +
  ...
  labs(title = "Stage vs. Discharge - DVNWT5", x = "Stage (LEVEL m)", y = "Discharge (Q m³/s)") +
  theme_minimal()

###########################################
#### Check for Log-Linear Relationship ####
###########################################
ggplot(rating_data, aes(x = log(Final_Water_Depth_m), y = log(Q.m3s))) +
  geom_point(color = "blue") +
...
#### Linear model? ####
#######################
linear_model <- lm(Q.m3s ~ Final_Water_Depth_m, data = rating_data)
...
lines(rating_data$Final_Water_Depth_m, pred_linear, col = "green", lwd = 2)
...
# predict discharge for the entire dataset
DVNWT5 <- DVNWT5 %>%
  mutate(Predicted_Discharge_Linear_m3s = a_linear + b_linear * Final_Water_Depth_m)

#############################
#### Compare predictions ####
#############################
# visualize predictions
plot(DVNWT5$Final_Water_Depth_m, DVNWT5$Predicted_Discharge_Log_m3s, col = "red", type = "l", lwd = 2,
     xlab = "Stage (m)", ylab = "Discharge (m³/s)", main = "Discharge Predictions")
lines(DVNWT5$Final_Water_Depth_m, DVNWT5$Predicted_Discharge_Linear_m3s, col = "green", lwd = 2)
...
ggplot(DVNWT5, aes(x = Final_Water_Depth_m)) +
...
######################################
DVNWT5$DateTime <- as.POSIXct(DVNWT5$DateTime)

ggplot(DVNWT5, aes(x = DateTime, y = Predicted_Discharge_Log_m3s)) +
  geom_line(color = "blue") +
  labs(title = "Predicted Discharge DVNWT5", x = "DateTime", y = "Discharge (m3/s)") +
...
#### Save file ####
###################
write.csv(DVNWT5, "data/discharge_DVNWT5.csv")
...
drive_put(
  media = "data/discharge_DVNWT5.csv",
