...
DVSB2$Baro_backup <- DVSB2$Baro_Cor_Lvl.m

########################################
#### Plot pressure compensated data ####
########################################
ggplot(data = DVSB2, aes(x = DateTime, y = Baro_Cor_Lvl.m)) +
...
ggplot(DVSB2, aes(x = DateTime, y = Baro_Cor_Lvl.m)) +
  geom_line() +
  geom_vline(xintercept = as.POSIXct("2024-11-19"), linetype="dashed", color="red") + # cut ziptie loop
  geom_vline(xintercept = as.POSIXct("2025-02-28"), linetype="dashed", color="red") + # Levelogger burried upon arrival. Adjusted up by 9.0 cm. Levelogger redeployed at 11:30:00 AM.
  geom_vline(xintercept = as.POSIXct("2025-06-03"), linetype="dashed", color="red") + # Level logger moved down 1.0 cm using the fence post pounder
...
Date2 <- as.Date("2025-07-02", "%Y-%m-%d")
DVSB2$Baro_Cor_Lvl.m[DVSB2$DateTime >= Date1 & DVSB2$DateTime <= Date2] <- NA
# remove noisy data
Date1 <- as.Date("2025-07-01", "%Y-%m-%d")
Date2 <- as.Date("2025-08-14", "%Y-%m-%d")
DVSB2$Baro_Cor_Lvl.m[DVSB2$DateTime >= Date1 & DVSB2$DateTime <= Date2] <- NA
...
# known offset correction (2024-11-19, 3.60 cm)
move_time3 <- as.POSIXct("2024-11-19 15:05:00")
offset3 <- -0.036  # m offset from field notes
...
# move_time4 <- as.POSIXct("2024-08-06 11:35:00")
# before_move4 <- DVSB2 %>%
...
# move_time5 <- as.POSIXct("2024-09-06 13:00:00")
# before_move5 <- DVSB2 %>%
...
ggplot(DVSB2, aes(x = DateTime, y = Final_Corrected_Lvl)) +
  geom_line() +
  labs(title = "Corrected Baro_Cor Over Time (Final Correction)", x = "Date", y = "Water Level (m)")
