...
## produce a true Final_Water_Depth_m timeseries for DVWT3.
##
## INPUT:  "offset" Google Drive folder (output of script 05) -> offset_DVWT3.csv
...
googledrive::drive_download(
  file = pt_csvs$id[pt_csvs$name == "offset_DVWT3.csv"],
...
DVWT3$Final_Corrected_Lvl  <- suppressWarnings(as.numeric(DVWT3$Final_Corrected_Lvl))
DVWT3$Actual_Water_Depth_m <- suppressWarnings(as.numeric(DVWT3$Actual_Water_Depth_m))

cat("Final_Corrected_Lvl NAs:  ", sum(is.na(DVWT3$Final_Corrected_Lvl)), "\n")
cat("Actual_Water_Depth_m NAs: ", sum(is.na(DVWT3$Actual_Water_Depth_m)), "\n")
...
cat("Number of paired depth observations for DVWT3:", nrow(level_data), "\n")
...
if (nrow(level_data) >= min_obs_for_model) {
  ...
  cat("\n--- Depth conversion model summary (DVWT3) ---\n")
  ...
  if (is.na(sensor_height_m)) {
    stop("Not enough paired observations to fit a depth model for DVWT3,
...
ggplot(level_data, aes(x = Actual_Water_Depth_m, y = Predicted_Depth_m)) +
  ...
  labs(
    title = "DVWT3: Observed vs. Predicted Water Depth",
...
ggplot(level_data, aes(x = Final_Corrected_Lvl, y = Actual_Water_Depth_m)) +
  ...
  geom_text(aes(label = format(as.Date(Date.x), "%Y-%m-%d")), vjust = -0.7, size = 3) +
  labs(
    title = "DVWT3: Offset-corrected pressure vs. actual field depth",
...
ggplot(DVWT3, aes(x = DateTime, y = Final_Water_Depth_m)) +
  geom_line(color = "steelblue") +
  labs(
    title = "DVWT3: Water Depth timeseries (converted from pressure)",
    x = "Date",
    y = "Water Depth (m)"
  ) +
  theme_minimal()
...
ggplot(DVWT3, aes(x = DateTime)) +
  ...
  labs(
    title = "DVWT3: Pressure vs. depth comparison",
...
n_negative <- sum(DVWT3$Final_Water_Depth_m < 0, na.rm = TRUE)
if (n_negative > 0) {
  cat(sprintf(
...
#### Save file ####
###################
write.csv(DVWT3, "data/depth_corrected_DVWT3.csv", row.names = FALSE)

# this is the "depth_corrected" folder — create this folder in Drive first
# and paste its ID here (reusing the folder originally set up for DVWT3)
drive_folder_id <- "1wlpAOSpJyB6LI2XYkRXRZwhXRkXqe5eG"  # <-- confirm/replace with your Drive folder ID

drive_put(
  media = "data/depth_corrected_DVWT3.csv",
