##==============================================================================
## Project: QuEST
## This script is to calculate discharge from compensated pressure data for Santa Fe DVSB1 site
##==============================================================================
...
# DVSB1
googledrive::drive_download(file = pt_csvs$id[pt_csvs$name=="depth_corrected_DVSB1.csv"],
                            path = "googledrive/depth_corrected_DVSB1.csv",
                            overwrite = T)
# load file
DVSB1 <- read.csv("googledrive/depth_corrected_DVSB1.csv")

# convert Date column to Date type if not already
DVSB1$Date <- as.Date(DVSB1$Date.x)
# combine Date and Time columns into a new DateTime column
DVSB1$DateTime <- paste(DVSB1$Date.x, DVSB1$TimeOnly, sep = " ")
...
########################################
ggplot(data = DVSB1, aes(x = DateTime, y = Final_Water_Depth_m)) +
...
plot(DVSB1$Final_Water_Depth_m, DVSB1$Predicted_Discharge_Log_m3s, col = "red", type = "l", lwd = 2,
     xlab = "Stage (m)", ylab = "Discharge (m³/s)", main = "Discharge Predictions")
lines(DVSB1$Final_Water_Depth_m, DVSB1$Predicted_Discharge_Linear_m3s, col = "green", lwd = 2)
...
ggplot(DVSB1, aes(x = Final_Water_Depth_m)) +
...
######################################
DVSB1$DateTime <- as.POSIXct(DVSB1$DateTime)
...
#### Save file ####
###################
write.csv(DVSB1, "data/discharge_DVSB1.csv")
...
drive_put(
  media = "data/discharge_DVSB1.csv",
