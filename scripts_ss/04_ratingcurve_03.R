...
#SST03
googledrive::drive_download(file = pt_csvs$id[pt_csvs$name=="2024-12-16_SST03_PTS_SN2192627.csv"], 
                            path = "googledrive/2024-12-16_SST03_PTS_SN2192627.csv",
                            overwrite = T)
# Load file
SST03 <- read.csv("googledrive/2024-12-16_SST03_PTS_SN2192627.csv")

# Convert Date column to Date type if not already
SST03$Date <- as.Date(SST03$Date.x)
SST03$DateTime <- as.POSIXct(SST03$DateTime, format = "%Y-%m-%d %H:%M:%S", tz = "MST")
head(SST03)
...
plot(SST03$Baro_Cor_Lvl.m, SST03$Predicted_Discharge_Log, col = "red", type = "l", lwd = 2,
     xlab = "Stage (m)", ylab = "Discharge (m³/s)", main = "Discharge Predictions")
lines(SST03$Baro_Cor_Lvl.m, SST03$Predicted_Discharge_Linear, col = "green", lwd = 2)
lines(SST03$Baro_Cor_Lvl.m, SST03$Predicted_Discharge_Poly, col = "purple", lwd = 2)
...
ggplot(SST03, aes(x = Baro_Cor_Lvl.m)) +
...
ggplot(SST03, aes(x = DateTime, y = Predicted_Discharge_Poly)) +
  geom_point(color = "blue") +
