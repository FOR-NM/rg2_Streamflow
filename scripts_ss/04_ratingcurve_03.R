##==============================================================================
## Project: QuEST
## This script is to calculate discharge from compensated pressure data for South Sandy SST03 site
## This site still needs to be accounted for two moves in Aug and Sept
...
#SST03
googledrive::drive_download(file = pt_csvs$id[pt_csvs$name=="offset_SST03.csv"], 
                            path = "googledrive/offset_SST03.csv",
                            overwrite = T)
# load file
SST03 <- read.csv("googledrive/offset_SST03.csv")

# convert Date column to Date type if not already
SST03$Date <- as.Date(SST03$Date.x)
SST03$DateTime <- paste(SST03$Date, SST03$Time.x, sep = " ")
...
rating_data <- SST03 %>% 
  filter(!is.na(Baro_Cor_offset2), !is.na(Q..L.s.))
...
ggplot(rating_data, aes(x = Depth.at.PT..cm., y = Q.m3s)) +
  geom_point(color = "blue") +
  geom_text(aes(label = Date.x), vjust = -0.5, size = 3) +  # adds date labels above points
...
plot(SST03$Baro_Cor_offset2, SST03$Predicted_Discharge_Log, col = "red", type = "l", lwd = 2,
     xlab = "Stage (m)", ylab = "Discharge (m³/s)", main = "Discharge Predictions")
lines(SST03$Baro_Cor_offset2, SST03$Predicted_Discharge_Linear, col = "green", lwd = 2)
legend("topleft", legend = c("Log-Transformed", "Linear"),
       col = c("red", "green"), lty = 1, lwd = 2)

# discharge from L/s to m3/s for entire data set
...
ggplot(SST03, aes(x = Baro_Cor_offset2)) +
...
######################################
SST03$DateTime <- as.POSIXct(SST03$DateTime)

ggplot(SST03, aes(x = DateTime, y = Predicted_Discharge_Log)) +
  geom_line(color = "blue") +
  labs(title = "Predicted Discharge (Log)", x = "DateTime", y = "Discharge (m3/s)") +
  scale_x_datetime(date_breaks = "1 week") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))

ggplot(SST03, aes(x = DateTime, y = Predicted_Discharge_Linear)) +
  geom_line(color = "blue") +
  labs(title = "Predicted Discharge (Linear)", x = "DateTime", y = "Discharge (m3/s)") +
  scale_x_datetime(date_breaks = "1 week") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))

# raw baro compensated data
ggplot(SST03, aes(x = DateTime, y = Baro_Cor_Lvl.m)) +
  geom_line(color = "blue") +
  labs(title = "Baro compensated level SST03", x = "DateTime") +
  scale_x_datetime(date_breaks = "1 week") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
...
#### Save file ####
###################
write.csv(SST03, "data/discharge_SST03.csv")

drive_folder_id <- "1tkYDtcrI_wgbb16zufLJizcjfMzePtkw"

# upload file to the specified Google Drive folder
drive_put(
  media = "data/discharge_SST03.csv",
