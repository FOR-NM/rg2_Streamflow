##==============================================================================
## Project: QuEST
## This script is to calculate PT offset for South Sandy SST08 site
...
#SST08
googledrive::drive_download(file = pt_csvs$id[pt_csvs$name=="SST08.csv"], 
                            path = "googledrive/SST08.csv",
                            overwrite = T)
# load file
SST08 <- read.csv("googledrive/SST08.csv")

# convert Date column to Date type if not already
SST08$Date <- as.Date(SST08$Date)
...
########################################
ggplot(data = SST08, aes(x = DateTime, y = Baro_Cor_Lvl.m)) +
...
#################################################
ggplot(SST08, aes(x = DateTime, y = Baro_Cor_Lvl.m)) +
  geom_line() +
...
subdf <- SST08[SST08$DateTime < Date2 & SST08$DateTime > Date1,]
...
subdf <- SST08[SST08$DateTime < Date2 & SST08$DateTime > Date1,]
ggplot(data=subdf, aes(DateTime,Baro_Cor_Lvl.m)) + geom_line() +
  geom_vline(xintercept = as.POSIXct("2024-12-16 09:45:00"), linetype="dashed", color="red") 

Date1 <- as.Date("2025-04-22", "%Y-%m-%d")
Date2 <- as.Date("2025-04-25", "%Y-%m-%d")
subdf <- SST08[SST08$DateTime < Date2 & SST08$DateTime > Date1,]
ggplot(data=subdf, aes(DateTime,Baro_Cor_Lvl.m)) + geom_line() +
  geom_vline(xintercept = as.POSIXct("2025-04-23 10:30:00"), linetype="dashed", color="red") 
...
SST08$Baro_Cor_Lvl.m[SST08$DateTime >= Date1 & SST08$DateTime <= Date2] <- NA
...
move_time1 <- as.POSIXct("2024-09-16 10:15:00")
...
move_time2 <- as.POSIXct("2024-12-16 10:00:00")
...
move_time3 <- as.POSIXct("2025-04-23 10:45:00")
...
ggplot(SST08, aes(x = DateTime, y = Baro_Cor_offset1)) +
  geom_line() +
  labs(title = "Corrected Baro_Cor Over Time (First Correction)", x = "Date", y = "Water Level (m)")
ggplot(SST08, aes(x = DateTime, y = Baro_Cor_offset2)) +
  geom_line() +
  labs(title = "Corrected Baro_Cor Over Time (Second Correction)", x = "Date", y = "Water Level (m)")
ggplot(SST08, aes(x = DateTime, y = Baro_Cor_offset3)) +
  geom_line() +
  labs(title = "Corrected Baro_Cor Over Time (Second Correction)", x = "Date", y = "Water Level (m)")

# discharge from L/s to m3/s in whole data set
SST08$Q..L.s. <- as.numeric(SST08$Q..L.s.)
SST08 <- SST08 %>%
  mutate(Q.m3s = Q..L.s./1000)

# filter out rows with missing stage or discharge
new_rating_data <- SST08 %>% 
...
#### Save file ####
###################
write.csv(SST08, "data/offset_SST08.csv")

drive_folder_id <- "1GcRdU4UaBrxEmVhZYoZuTae6HxVZt3Gm"

# upload file to the specified Google Drive folder
drive_put(
  media = "data/offset_SST08.csv",
