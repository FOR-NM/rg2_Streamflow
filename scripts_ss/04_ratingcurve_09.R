##==============================================================================
## Project: QuEST
## This script is to calculate discharge from compensated pressure data for South Sandy SST09 site
...
#SST09
googledrive::drive_download(file = pt_csvs$id[pt_csvs$name=="offset_SST09.csv"], 
                            path = "googledrive/offset_SST09.csv",
                            overwrite = T)
# load file
SST09 <- read.csv("googledrive/offset_SST09.csv")
...
rating_data <- SST09 %>% 
  filter(!is.na(Baro_Cor_offset1), !is.na(Q..L.s.))
...
########################################
ggplot(data = SST09, aes(x = DateTime, y = Baro_Cor_offset1)) +
...
SST09 <- SST09 %>%
  mutate(Predicted_Discharge_Log = a_log * (Baro_Cor_offset1 ^ b_log))
...
# predict discharge for the entire dataset
SST09 <- SST09 %>%
  mutate(Predicted_Discharge_Linear = a_linear + b_linear * Baro_Cor_offset1)

#############################
#### Compare predictions ####
#############################
# visualize predictions
plot(SST09$Baro_Cor_offset1, SST09$Predicted_Discharge_Log, col = "red", type = "l", lwd = 2,
     xlab = "Stage (m)", ylab = "Discharge (m³/s)", main = "Discharge Predictions")
lines(SST09$Baro_Cor_offset1, SST09$Predicted_Discharge_Linear, col = "green", lwd = 2)
...
ggplot(SST09, aes(x = Baro_Cor_offset1)) +
...
######################################
SST09$DateTime <- as.POSIXct(SST09$DateTime)

ggplot(SST09, aes(x = DateTime, y = Predicted_Discharge_Log)) +
  geom_line(color = "blue") +
  labs(title = "Predicted Discharge (Log)", x = "DateTime", y = "Discharge (m3/s)") +
  scale_x_datetime(date_breaks = "1 week") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))

ggplot(SST09, aes(x = DateTime, y = Predicted_Discharge_Linear)) +
  geom_line(color = "blue") +
...
#### Save file ####
###################
write.csv(SST09, "data/discharge_SST09.csv")

drive_folder_id <- "1tkYDtcrI_wgbb16zufLJizcjfMzePtkw"

# upload file to the specified Google Drive folder
drive_put(
  media = "data/discharge_SST09.csv",
