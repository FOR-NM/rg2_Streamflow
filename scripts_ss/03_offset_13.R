...
#SST13
googledrive::drive_download(file = pt_csvs$id[pt_csvs$name=="SST13.csv"], 
                            path = "googledrive/SST13.csv",
                            overwrite = T)
# load file
SST13 <- read.csv("googledrive/SST13.csv")
...
subdf <- SST13[SST13$DateTime < Date2 & SST13$DateTime > Date1,]
ggplot(data=subdf, aes(DateTime,Baro_Cor_Lvl.m)) + geom_line() +
  geom_vline(xintercept = as.POSIXct("2024-09-16 11:15:00"), linetype="dashed", color="red") 
...
subdf <- SST13[SST13$DateTime < Date2 & SST13$DateTime > Date1,]
ggplot(data=subdf, aes(DateTime,Baro_Cor_Lvl.m)) + geom_line() +
  geom_vline(xintercept = as.POSIXct("2025-04-23 11:30:00"), linetype="dashed", color="red") 
...
Date1 <- as.Date("2024-06-17", "%Y-%m-%d")
Date2 <- as.Date("2024-06-19", "%Y-%m-%d")
SST13$Baro_Cor_Lvl.m[SST13$DateTime >= Date1 & SST13$DateTime <= Date2] <- NA
...
move_time1 <- as.POSIXct("2024-09-16 11:15:00")
...
move_time2 <- as.POSIXct("2024-12-16 10:45:00")
...
move_time3 <- as.POSIXct("2025-04-23 11:30:00")
...
ggplot(SST13, aes(x = DateTime, y = Baro_Cor_offset1)) +
  geom_line() +
  labs(title = "Corrected Baro_Cor Over Time (First Correction)", x = "Date", y = "Water Level (m)")
...
ggplot(SST13, aes(x = DateTime, y = Baro_Cor_offset3)) +
  geom_line() +
  labs(title = "Corrected Baro_Cor Over Time (Second Correction)", x = "Date", y = "Water Level (m)")

# discharge from L/s to m3/s in whole data set
SST13$Q..L.s. <- as.numeric(SST13$Q..L.s.)
SST13 <- SST13 %>%
  mutate(Q.m3s = Q..L.s./1000)

# filter out rows with missing stage or discharge
new_rating_data <- SST13 %>% 
