##==============================================================================
## Project: QuEST
## This script is to calculate discharge from compensated pressure data for South Sandy SST13 site
...
#SST13
googledrive::drive_download(file = pt_csvs$id[pt_csvs$name=="09-16-2024_SST13_PTS_SN2192886.csv"], 
                            path = "googledrive/09-16-2024_SST13_PTS_SN2192886.csv",
                            overwrite = T)
# Load file
SST13 <- read.csv("googledrive/09-16-2024_SST13_PTS_SN2192886.csv")

# Convert Date column to Date type if not already
SST13$Date <- as.Date(SST13$Date.x)
...
lines(SST13$Baro_Cor_Lvl.m, SST13$Predicted_Discharge_Poly, col = "purple", lwd = 2)
...
SST13$DateTime <- as.POSIXct(SST13$DateTime)

ggplot(SST13, aes(x = DateTime, y = Predicted_Discharge_Log)) +
  geom_point(color = "blue") +
  labs(title = "Predicted Discharge (Log)", x = "DateTime", y = "Discharge (m3/s)") +
  scale_x_datetime(date_breaks = "1 week") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))

ggplot(SST13, aes(x = DateTime, y = Predicted_Discharge_Linear)) +
  geom_point(color = "blue") +
  labs(title = "Predicted Discharge (Linear)", x = "DateTime", y = "Discharge (m3/s)") +
  scale_x_datetime(date_breaks = "1 week") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))

ggplot(SST13, aes(x = DateTime, y = Predicted_Discharge_Poly)) +
  geom_point(color = "blue") +
...
drive_put(
  media = "data/discharge_SST13.csv",
