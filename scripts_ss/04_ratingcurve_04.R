##==============================================================================
## Project: QuEST
## This script is to calculate discharge from compensated pressure data for South Sandy SST04 site
...
#SST04
googledrive::drive_download(file = pt_csvs$id[pt_csvs$name=="offset_SST04.csv"], 
                            path = "googledrive/offset_SST04.csv",
                            overwrite = T)
# load file
SST04 <- read.csv("googledrive/offset_SST04.csv")

#convert Date column to Date type if not already
SST04$Date <- as.Date(SST04$Date.x)
SST04$DateTime <- paste(SST04$Date, SST04$Time.x, sep = " ")
# convert the DateTime column to POSIXct
SST04$DateTime <- as.POSIXct(SST04$DateTime, format = "%Y-%m-%d %I:%M:%S %p")
...
# pt depth from cm to m# pt Baro_Cor_offset2 depth from cm to m
rating_data <- rating_data %>%
  mutate(pt_depth_m = Depth.above.PT..cm....7)
...
plot(SST04$Baro_Cor_offset2, SST04$Predicted_Discharge_Log, col = "red", type = "l", lwd = 2,
     xlab = "Stage (m)", ylab = "Discharge (m³/s)", main = "Discharge Predictions")
lines(SST04$Baro_Cor_offset2, SST04$Predicted_Discharge_Linear, col = "green", lwd = 2)
...
ggplot(SST04, aes(x = Baro_Cor_offset2)) +
...
######################################
SST04$DateTime <- as.POSIXct(SST04$DateTime)

ggplot(SST04, aes(x = DateTime, y = Predicted_Discharge_Log)) +
  geom_line(color = "blue") +
  labs(title = "Predicted Discharge (Log)", x = "DateTime", y = "Discharge (m3/s)") +
  scale_x_datetime(date_breaks = "1 week") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))

ggplot(SST04, aes(x = DateTime, y = Predicted_Discharge_Linear)) +
  geom_line(color = "blue") +
...
write.csv(SST04, "data/discharge_SST04.csv")
