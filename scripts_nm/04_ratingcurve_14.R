##==============================================================================
## Project: QuEST
## This script is to calculate discharge from compensated pressure data for Santa Fe USF14 site
...
USF14$Date <- as.Date(USF14$Date.x)
USF14$DateTime <- as.POSIXct(USF14$DateTime, format = "%Y-%m-%d %H:%M:%S", tz = "MST")
head(USF14)
...
plot(USF14$Baro_Cor_Lvl, USF14$Predicted_Discharge_Log, col = "red", type = "l", lwd = 2,
     xlab = "Stage (m)", ylab = "Discharge (m³/s)", main = "Discharge Predictions")
lines(USF14$Baro_Cor_Lvl, USF14$Predicted_Discharge_Linear, col = "green", lwd = 2)
lines(USF14$Baro_Cor_Lvl, USF14$Predicted_Discharge_Poly, col = "purple", lwd = 2)
...
ggplot(USF14, aes(x = Baro_Cor_Lvl)) +
...
USF14$DateTime <- as.POSIXct(USF14$DateTime)

ggplot(USF14, aes(x = DateTime, y = Predicted_Discharge_Log)) +
  geom_point(color = "blue") +
  labs(title = "Predicted Discharge (Log)", x = "DateTime", y = "Discharge (m3/s)") +
  scale_x_datetime(date_breaks = "1 week") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))

ggplot(USF14, aes(x = DateTime, y = Predicted_Discharge_Linear)) +
  geom_point(color = "blue") +
  labs(title = "Predicted Discharge (Linear)", x = "DateTime", y = "Discharge (m3/s)") +
  scale_x_datetime(date_breaks = "1 week") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))

ggplot(USF14, aes(x = DateTime, y = Predicted_Discharge_Poly)) +
  geom_point(color = "blue") +
...
write.csv(USF14, "data/discharge_USF14.csv")

drive_folder_id <- "1fPDNinUQ3pCFFQXJ1dtLGbqEawyTmPUx"

# Upload file to the specified Google Drive folder
drive_put(
  media = "data/discharge_USF14.csv",
