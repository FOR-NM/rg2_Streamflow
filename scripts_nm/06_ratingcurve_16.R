##==============================================================================
## Project: QuEST
## This script is to calculate discharge from compensated pressure data for Santa Fe USF16 site
...
#USF16
googledrive::drive_download(file = pt_csvs$id[pt_csvs$name=="offset_USF16.csv"], 
                            path = "googledrive/offset_USF16.csv",
                            overwrite = T)
# load file
USF16 <- read.csv("googledrive/offset_USF16.csv")

# convert Date column to Date type if not already
USF16$Date <- as.Date(USF16$Date.x)
# combine Date and Time columns into a new DateTime column
USF16$DateTime <- paste(USF16$Date, USF16$Time, sep = " ")
...
########################################
ggplot(data = USF16, aes(x = DateTime, y = Baro_Cor_offset1)) +
...
plot(USF16$Baro_Cor_offset1, USF16$Predicted_Discharge_Log_m3s, col = "red", type = "l", lwd = 2,
     xlab = "Stage (m)", ylab = "Discharge (m³/s)", main = "Discharge Predictions")
lines(USF16$Baro_Cor_offset1, USF16$Predicted_Discharge_Linear_m3s, col = "green", lwd = 2)
...
ggplot(USF16, aes(x = Baro_Cor_offset1)) +
...
######################################
USF16$DateTime <- as.POSIXct(USF16$DateTime)

ggplot(USF16, aes(x = DateTime, y = Predicted_Discharge_Log_m3s)) +
  geom_point(color = "blue") +
  labs(title = "Predicted Discharge (Log)", x = "DateTime", y = "Discharge (m3/s)") +
  scale_x_datetime(date_breaks = "2 week") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))

ggplot(USF16, aes(x = DateTime, y = Predicted_Discharge_Linear_m3s)) +
  geom_point(color = "blue") +
...
#### Save file ####
###################
write.csv(USF16, "data/discharge_USF16.csv")
...
drive_put(
  media = "data/discharge_USF16.csv",
