...
## Script 06: PT offset corrections for NM USF16 (upper, AIR2 baro source)
##
...
## OUTPUT: "offset" folder -> used by 07_ratingcurve_USF16.R
##==============================================================================
...
googledrive::drive_download(file = pt_csvs$id[pt_csvs$name == paste0(site, ".csv")],
                            ...
                            overwrite = TRUE)
USF16 <- read.csv(file.path("googledrive", paste0(site, ".csv")))

# DateTime at midnight is missing 00:00:00 time in this file, so fill it in
USF16$DateTime[grep("[0-9]{4}-[0-9]{2}-[0-9]{2}$", USF16$DateTime)] <- paste(
  USF16$DateTime[grep("[0-9]{4}-[0-9]{2}-[0-9]{2}$", USF16$DateTime)], "00:00:00")
USF16$DateTime <- as.POSIXct(USF16$DateTime, format = "%Y-%m-%d %H:%M:%S")

########################################
#### Plot pressure compensated data ####
########################################
ggplot(USF16, aes(x = DateTime, y = Baro_Cor_Lvl.m)) +
  geom_line() +
  geom_vline(xintercept = as.POSIXct(c("2024-10-29 14:15:00", "2025-06-05 12:43:36")),
...
###################################################################
out_of_water_times <- as.POSIXct(c("2024-10-29 13:00:00", "2024-10-29 12:30:00", "2024-10-29 13:15:00", "2024-10-29 13:30:00", "2024-10-29 13:45:00", "2024-10-29 14:00:00", "2025-06-05 12:30:00"))
...
USF16$Baro_Cor_offset0 <- USF16$Baro_Cor_Lvl.m

move_time1 <- as.POSIXct("2024-10-29 14:15:00")
offset1 <- apply_offset(USF16, "Baro_Cor_offset0", move_time1)
USF16 <- USF16 %>%
  mutate(Baro_Cor_offset1 = if_else(DateTime >= move_time1, Baro_Cor_offset0 - offset1, Baro_Cor_offset0))

move_time2 <- as.POSIXct("2025-06-05 12:45:")
offset2 <- apply_offset(USF16, "Baro_Cor_offset1", move_time2)
USF16 <- USF16 %>%
...
##############################
ggplot(USF16, aes(x = DateTime, y = Final_Corrected_Lvl)) +
  geom_line() +
...
frozen_start <- as.POSIXct("2024-12-10 00:00:00")  # set to NA if no frozen period
frozen_end   <- as.POSIXct("2024-12-21 00:00:00")
...
#### Save file ####
###################
write.csv(USF16, paste0("data/offset_", site, ".csv"), row.names = FALSE, quote = FALSE)
