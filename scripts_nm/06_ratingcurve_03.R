...
# filter out data after big storm in August 2025. That needs a different rating curve
rating_data <- USF03 %>% 
  filter(!(Date.y == "2025-09-12"), !is.na(Q))
...
########################################
ggplot(data = USF03, aes(x = DateTime, y = Baro_Cor_offset3)) +
  geom_line() + ggtitle("USF03 compensated level data")

##################################
#### Plot Stage vs. Discharge ####
##################################
ggplot(rating_data, aes(x = Baro_Cor_offset3, y = Q)) +
...
plot(USF03$Baro_Cor_offset3, USF03$Predicted_Discharge_Log_m3s, col = "red", type = "l", lwd = 2,
     xlab = "Stage (m)", ylab = "Discharge (m³/s)", main = "Discharge Predictions")
lines(USF03$Baro_Cor_offset3, USF03$Predicted_Discharge_Linear_m3s, col = "green", lwd = 2)
...
ggplot(USF03, aes(x = Baro_Cor_offset3)) +
