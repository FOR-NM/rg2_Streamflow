##==============================================================================
## Project: QuEST
## This script is to calculate discharge from compensated pressure data for Santa Fe USF04 site
...
#USF04
googledrive::drive_download(file = pt_csvs$id[pt_csvs$name=="offset_USF04.csv"], 
                            path = "googledrive/offset_USF04.csv",
                            overwrite = T)
# load file
USF04 <- read.csv("googledrive/offset_USF04.csv")

# convert Date column to Date type if not already
USF04$Date <- as.Date(USF04$Date.x)
...
########################################
ggplot(data = USF04, aes(x = DateTime, y = Baro_Cor_offset1)) +
...
plot(USF04$Baro_Cor_offset1, USF04$Predicted_Discharge_Log_m3s, col = "red", type = "l", lwd = 2,
     xlab = "Stage (m)", ylab = "Discharge (m³/s)", main = "Discharge Predictions")
lines(USF04$Baro_Cor_offset1, USF04$Predicted_Discharge_Linear_m3s, col = "green", lwd = 2)
...
ggplot(USF04, aes(x = Baro_Cor_offset1)) +
...
######################################
USF04$DateTime <- as.POSIXct(USF04$DateTime)

ggplot(USF04, aes(x = DateTime, y = Predicted_Discharge_Log_m3s)) +
  geom_point(color = "blue") +
  labs(title = "Predicted Discharge (Log)", x = "DateTime", y = "Discharge (m3/s)") +
  scale_x_datetime(date_breaks = "2 week") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))

ggplot(USF04, aes(x = DateTime, y = Predicted_Discharge_Linear_m3s)) +
  geom_point(color = "blue") +
...
#### Save file ####
###################
write.csv(USF04, "data/discharge_USF04.csv")
...
drive_put(
  media = "data/discharge_USF04.csv",
