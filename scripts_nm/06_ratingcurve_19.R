##==============================================================================
## Project: QuEST
## This script is to calculate discharge from compensated pressure data for Santa Fe USF19 site
...
#USF19
googledrive::drive_download(file = pt_csvs$id[pt_csvs$name=="offset_USF19.csv"], 
                            path = "googledrive/offset_USF19.csv",
                            overwrite = T)
# load file
USF19 <- read.csv("googledrive/offset_USF19.csv")

# convert Date column to Date type if not already
USF19$Date <- as.Date(USF19$Date.x)
# combine Date and Time columns into a new DateTime column
USF19$DateTime <- paste(USF19$Date, USF19$Time, sep = " ")
...
rating_data <- USF19 %>% 
  filter(!is.na(Baro_positive), !is.na(Q))
...
##################################
ggplot(rating_data, aes(x = Baro_positive, y = Q)) +
...
ggplot(rating_data, aes(x = Baro_positive, y = Q.m3s)) +
  geom_point(color = "blue") +
  labs(title = "Stage vs. Discharge", x = "Stage (LEVEL.m)", y = "Discharge (Q)") +
  theme_minimal()
...
###########################################
ggplot(rating_data, aes(x = log(Baro_positive), y = log(Q))) +
  geom_point(color = "blue") +
...
plot(USF19$Baro_positive, USF19$Predicted_Discharge_Log_m3s, col = "red", type = "l", lwd = 2,
     xlab = "Stage (m)", ylab = "Discharge (m³/s)", main = "Discharge Predictions")
lines(USF19$Baro_positive, USF19$Predicted_Discharge_Linear_m3s, col = "green", lwd = 2)
...
ggplot(USF19, aes(x = Baro_positive)) +
...
######################################
USF19$DateTime <- as.POSIXct(USF19$DateTime)

ggplot(USF19, aes(x = DateTime, y = Predicted_Discharge_Log_m3s)) +
  geom_point(color = "blue") +
  labs(title = "Predicted Discharge (Log)", x = "DateTime", y = "Discharge (m3/s)") +
  scale_x_datetime(date_breaks = "2 week") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))

ggplot(USF19, aes(x = DateTime, y = Predicted_Discharge_Linear_m3s)) +
  geom_point(color = "blue") +
...
#### Save file ####
###################
write.csv(USF19, "data/discharge_USF19.csv")
...
drive_put(
  media = "data/discharge_USF19.csv",
