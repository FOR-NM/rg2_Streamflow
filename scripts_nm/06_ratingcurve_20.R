...
## Script 07: Rating curve and discharge prediction for NM USF20 (lower, AIR1 baro source)
##
...
googledrive::drive_download(file = pt_csvs$id[pt_csvs$name == paste0("offset_", site, ".csv")],
                            ...
                            overwrite = TRUE)
USF20 <- read.csv(file.path("googledrive", paste0("offset_", site, ".csv")))
...
#############################################
p_ts <- ggplot() +
  geom_line(data = USF20, aes(x = DateTime, y = Q_m3s_pred), linewidth = 1.0, color = "black") +
...
diag  <- add_rating_diagnostics(USF20, rating_data, log_model)
USF20 <- diag$df

###################
#### Save file ####
###################
write.csv(USF20, paste0("data/discharge_", site, ".csv"), row.names = FALSE, quote = FALSE)
