...
#DVMS5
googledrive::drive_download(file = pt_csvs$id[pt_csvs$name=="offset_DVMS5.csv"], 
...
rating_data <- DVMS5 %>% 
  filter(!is.na(Baro_Cor_offset5), !is.na(Q))
...
########################################
ggplot(data = DVMS5, aes(x = DateTime, y = Baro_Cor_offset5)) +
...
###########################################
ggplot(rating_data, aes(x = log(Baro_Cor_offset5), y = log(Q))) +
  geom_point(color = "blue") +
...
plot(DVMS5$Baro_Cor_offset5, DVMS5$Predicted_Discharge_Log_m3s, col = "red", type = "l", lwd = 2,
     xlab = "Stage (m)", ylab = "Discharge (m³/s)", main = "Discharge Predictions")
lines(DVMS5$Baro_Cor_offset5, DVMS5$Predicted_Discharge_Linear_m3s, col = "green", lwd = 2)
...
ggplot(DVMS5, aes(x = Baro_Cor_offset5)) +
