...
# DVNWT5
googledrive::drive_download(file = pt_csvs$id[pt_csvs$name=="offset_DVNWT5.csv"], 
...
rating_data <- DVNWT5 %>% 
  filter(!is.na(Baro_Cor_offset5), !is.na(Q), !Q == 0)
...
########################################
ggplot(data = DVNWT5, aes(x = DateTime, y = Baro_Cor_offset5)) +
...
DVNWT5 <- DVNWT5 %>%
  mutate(Predicted_Discharge_Log_m3s = a_log * (Baro_Cor_offset5 ^ b_log))
...
# predict discharge for the entire dataset
DVNWT5 <- DVNWT5 %>%
  mutate(Predicted_Discharge_Linear_m3s = a_linear + b_linear * Baro_Cor_offset5)

#############################
#### Compare predictions ####
#############################
# visualize predictions
plot(DVNWT5$Baro_Cor_offset5, DVNWT5$Predicted_Discharge_Log_m3s, col = "red", type = "l", lwd = 2,
     xlab = "Stage (m)", ylab = "Discharge (m³/s)", main = "Discharge Predictions")
lines(DVNWT5$Baro_Cor_offset5, DVNWT5$Predicted_Discharge_Linear_m3s, col = "green", lwd = 2)
...
ggplot(DVNWT5, aes(x = Baro_Cor_offset5)) +
