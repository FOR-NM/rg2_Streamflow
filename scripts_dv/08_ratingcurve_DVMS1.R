...
#DVMS1
googledrive::drive_download(file = pt_csvs$id[pt_csvs$name=="offset_DVMS1.csv"], 
...
DVMS1$DateTime <- paste(DVMS1$Date.x, DVMS1$Time.x, sep = " ")
...
rating_data <- DVMS1 %>% 
  filter(!is.na(Baro_Cor_offset3), !is.na(Q))
...
########################################
ggplot(data = DVMS1, aes(x = DateTime, y = Baro_Cor_offset3)) +
...
###########################################
ggplot(rating_data, aes(x = log(Baro_Cor_offset3), y = log(Q))) +
  geom_point(color = "blue") +
...
plot(DVMS1$Baro_Cor_offset3, DVMS1$Predicted_Discharge_Log_m3s, col = "red", type = "l", lwd = 2,
     xlab = "Stage (m)", ylab = "Discharge (m³/s)", main = "Discharge Predictions")
lines(DVMS1$Baro_Cor_offset3, DVMS1$Predicted_Discharge_Linear_m3s, col = "green", lwd = 2)
...
ggplot(DVMS1, aes(x = Baro_Cor_offset3)) +
...
ggplot(DVMS1, aes(x = DateTime, y = Predicted_Discharge_Linear_m3s)) +
  geom_point(color = "blue") +
